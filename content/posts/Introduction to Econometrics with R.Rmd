...
author:
  name: Ziwen
  affiliation: UCSD | Dept.of Political Science
...
```{r summarize and arrange}
top5=tot_pop%>%arrange(desc(counts))%>%
  top_n(5)
top5
...
sexes %>%
  ...
  facet_wrap(~sex)+theme_bw()   # make two panels, splitting by groups `sex`
```
...
merge_GER%>%
  ...
  ggplot(aes(x=year,y=prop_GER,color=Country,group=Country))+
  geom_line(size=1)+
